...
<div class="container-fluid col-md-9">
	...
	<div class="row">
		<table class="table table-hover">
			<thead>
				<tr>
					<th>NUM</th><th style="width : 10%;">TITLE</th><th><th>ID</th><th>DATE</th>
					...
					<c:if test="${member.roleDTO.roleNum eq 1 && boardName eq 'Qna'}">	
				    	<th>답변</th>
				   </c:if> 
				...
				<tr>
					...
					<c:if test="${boardName eq 'Qna' }">
						...
						<h5 style="font-weight: bold; "> [문의 내용] </h5> <p style="font-weight: bold; text-align: initial; background-color: #FFE794;"> ${dto.contents}</p>  
						   	<a href="../qnaReply/listdetail?num=${dto.num}"  class="btn btn-warning" >답변 보기</a>
					...
					<c:if test="${boardName eq 'QnaReply' }">
						<td>
						 	<p style="font-weight: bold;">${dto.contents}</p>
						</td>
					</c:if>
					...
					<c:if test="${member.id eq dto.id && boardName eq 'QnaReply'}">
						<td> 
						<form action="./delete" id="frmde">
						 <input type="hidden" name="replyNum" value="${dto.replyNum}">
						 <button  type="button" class="btn btn-warning delete" data-replynum="${dto.replyNum}" data-num="${dto.num}" >DELETE</button>
						</form>
						</td>
						<td>
						 <a class="btn btn-warning"  href ="./update?replyNum=${dto.replyNum}" data-replynum="${dto.replyNum}">UPDATE</a>
						</td>
					</c:if>
					
					<c:if test="${member.id eq dto.id && boardName eq 'Qna'}">
						<td> 
						<form action="./delete" id="frmde">
						 <input type="hidden" name="num" value="${dto.num}">
						 <button  type="button" class="btn btn-warning delete2" data-num="${dto.num}">DELETE</button>
						</form>
						</td>
						<td>
						 <a class="btn btn-warning"  href ="./update?num=${dto.num}" data-num="${dto.num}">UPDATE</a>
						</td>
					</c:if>
					...
					<c:if test="${boardName eq 'Qna'}">
		         	 ...
		         	 <c:if test="${member.roleDTO.roleNum eq 1 && boardName eq 'Qna' || dto.productDTO.id eq member.id && member.id ne null}">
		        	
		            <td width="86px" >
						<a href="/qnaReply/add?num=${dto.num}&productNum=${dto.productNum}" class="btn btn-warning">답변</a>
					</td> 
	...
	<c:if test="${boardName ne 'QnaReply' }">
		<!-- paging -->
		<div class="container">
